...
namespace ImGui
{
    ...
    struct MarkdownConfig
    {
        static const int        NUMHEADINGS = 6;
    ...
    inline void RenderLine(const char* markdown_, Line& line_, TextRegion& textRegion_, const MarkdownConfig& mdConfig_)
    {
        ...
        for (int j = indentStart; j < line_.leadSpaceCount; ++j)    // add indents
        {
            ImGui::Indent();
        }
        ...
        // unindent
        for (int j = indentStart; j < line_.leadSpaceCount; ++j)
        {
            ImGui::Unindent();
        }
    }
        ...
        for (int i = 0; i < (int)markdownLength_; ++i)
        {
            ...
            if (line.isLeadingSpace)
            {
                ...
                else
                {
                    ...
                    if ((c == '*' || c == '-') && (line.leadSpaceCount >= 0)) //was 2 inst 0
                    {
    ...
    inline void defaultMarkdownFormatCallback(const MarkdownFormatInfo& markdownFormatInfo_, bool start_)
    {
        ...
        case MarkdownFormatType::EMPHASIS:
        {
            ...
            if (markdownFormatInfo_.level == 1) {// italic
                if (auto font = markdownFormatInfo_.config->ItalicFont) start_ ? ImGui::PushFont(font) : ImGui::PopFont();
                else {
                    start_ ? ImGui::PushStyleColor(ImGuiCol_Text, ImGui::GetStyle().Colors[ImGuiCol_TextDisabled]) : ImGui::PopStyleColor();
                }
            } 
            else if (markdownFormatInfo_.level == 2) {// bold
                if (auto font = markdownFormatInfo_.config->BoldFont) start_ ? ImGui::PushFont(font) : ImGui::PopFont();
                else {
                    fmt = markdownFormatInfo_.config->headingFormats[MarkdownConfig::NUMHEADINGS - 1];
                    if (fmt.font) start_ ? ImGui::PushFont(fmt.font) : ImGui::PopFont();
                }
            }
            else {// italic bold
                if (auto font = markdownFormatInfo_.config->ItalicBoldFont) start_ ? ImGui::PushFont(font) : ImGui::PopFont();
                else {
                    // italic
                    start_ ? ImGui::PushStyleColor(ImGuiCol_Text, ImGui::GetStyle().Colors[ImGuiCol_TextDisabled]) : ImGui::PopStyleColor();
                    // bold
        ...
        {
            MarkdownHeadingFormat fmt = markdownFormatInfo_.level > MarkdownConfig::NUMHEADINGS 
                ? markdownFormatInfo_.config->headingFormats[MarkdownConfig::NUMHEADINGS - 1]
                : markdownFormatInfo_.config->headingFormats[markdownFormatInfo_.level - 1];
            //ImGui::NewLine();
            if (start_) {
                if (fmt.font) ImGui::PushFont(fmt.font);
            }
            else {
                if (fmt.separator) ImGui::Separator();
        ...
        case MarkdownFormatType::LINK:
            if (start_)
            {
                ...
                ImGui::PushStyleColor(ImGuiCol_Text, markdownFormatInfo_.itemHovered ? 
                    ImGui::GetStyle().Colors[ImGuiCol_TextSelectedBg] : ImGui::GetStyle().Colors[ImGuiCol_TextLink]
                );
            }
            else
            {
                ImGui::PopStyleColor();
                ImGui::UnderLine(markdownFormatInfo_.itemHovered ?
                    ImGui::GetStyle().Colors[ImGuiCol_TextSelectedBg] : ImGui::GetStyle().Colors[ImGuiCol_TextLink]
                );
            }
            break;
...
namespace ImGui {
    ...
    static ImFont* MDFont_H1;
    static ImFont* MDFont_H2;
    static ImFont* MDFont_H3;
    static ImFont* MDFont_H4;
    static ImFont* MDFont_H5;
    static ImFont* MDFont_H6;

    static ImFont* MDFont_Italic;
    static ImFont* MDFont_Bold;
    static ImFont* MDFont_ItalicBold;

    namespace MDImpl {

        inline void LinkCallback(ImGui::MarkdownLinkCallbackData data_) {
            std::string url(data_.link, data_.linkLength);
            if (!data_.isImage)geode::utils::web::openLinkInBrowser(url.c_str());
        }

        inline ImGui::MarkdownImageData ImageCallback(ImGui::MarkdownLinkCallbackData data_) {

            ImTextureID image = ImGui::GetIO().Fonts->TexNextUniqueID;
    ...
    void MDText(const std::string& markdown_, float maxWidth = ImGui::GetContentRegionAvail().x) {
        static std::map<std::string, geode::Ref<geode::MDTextArea>> created;
        matjson::Value id_val;
        id_val["markdown_"] = markdown_;
        id_val["maxWidth"] = maxWidth;
        auto id = id_val.dump();
        if (not created.contains(id)) {
            created[id] = geode::MDTextArea::create(
                markdown_, { ImGuiCocos::frameToCocos({maxWidth, 10.f}).x, 10.f }
            );
        }
        created[id]->getScrollLayer()->m_contentLayer->setPosition({ 0.f, 0.f });
        //  geode::log::debug("{}", created[id]);
        CCNodeImage(created[id]->getScrollLayer()->m_contentLayer, maxWidth);
    }
    ...
    inline void Markdown(const std::string& markdown_, MarkdownConfig mdConfig = MarkdownConfig()) {
        ...
        if (!mdConfig.tooltipCallback) mdConfig.tooltipCallback = defaultMarkdownTooltipCallback;
        //if (!mdConfig.imageCallback) mdConfig.imageCallback = MDImpl::ImageCallback;

        if (MDFont_H1 and !mdConfig.headingFormats[0].font) mdConfig.headingFormats[0] = { MDFont_H1, true };
        if (MDFont_H2 and !mdConfig.headingFormats[1].font) mdConfig.headingFormats[1] = { MDFont_H2, true };
        if (MDFont_H3 and !mdConfig.headingFormats[2].font) mdConfig.headingFormats[2] = { MDFont_H3, false };
        if (MDFont_H4 and !mdConfig.headingFormats[3].font) mdConfig.headingFormats[3] = { MDFont_H4, false };
        if (MDFont_H5 and !mdConfig.headingFormats[4].font) mdConfig.headingFormats[4] = { MDFont_H5, false };
        if (MDFont_H6 and !mdConfig.headingFormats[5].font) mdConfig.headingFormats[5] = { MDFont_H6, false };

        if (MDFont_Italic and !mdConfig.ItalicFont) mdConfig.ItalicFont = MDFont_Italic;
        if (MDFont_Bold and !mdConfig.BoldFont) mdConfig.BoldFont = MDFont_Bold;
        if (MDFont_ItalicBold and !mdConfig.ItalicBoldFont) mdConfig.ItalicBoldFont = MDFont_ItalicBold;
